...
def build_eepran_model_min_energy(link_file_path: str, node_file_path: str, route_file_path: str,
                                  is_ring_topology: bool, expected_centralization: int) -> {Model, AbstractConstraint}:
    model = Model(name='EEP-Ran Min Energy Problem', log_output=True)
    ...
    computing_core = process_computing_core(node_list)
    ...
    var_keys = [VarKey(route=route, drc=drc, ru=ru)
                ...
                for drc in drc_list
                for ru in computing_core.ru_list
                if route.is_destination(ru)
    ...
    for cr in computing_core.cr_dict:
        ...
        for hardware in computing_core.cr_dict[cr]:
            ...
            for fs in fs_list:
                dynamic_power_function += model.sum(model.x[key] * fs_list[fs] * hardware.dynamic_power / hardware.cpu
                                                    for key in var_keys
                ...
                psi_1 += model.sum(model.x[key]
                                   ...
                                   if key.route.contains(hardware) and (
                                           ...
                                           (key.route.is_du(hardware) and fs in key.drc.fs_du) or
                                           (key.route.is_ru(hardware) and fs in key.drc.fs_ru)))
            static_power_function += model.sum(model.min(1, psi_1) * hardware.static_power)
    ...
    centralization_constraint = model.add_constraint(centralization >= expected_centralization,
                                                     ctname='centralization constraint')
...
def build_eepran_model_max_centralization(link_file_path: str, node_file_path: str, route_file_path: str,
                                          is_ring_topology: bool, power_cap: float) -> {Model}:
    model = Model(name='EEP-Ran Max Centralization Problem', log_output=True)
            ...
            for fs in fs_list:
                ...
                psi_1 += model.sum(model.x[key]
                                   ...
                                   if key.route.contains(hardware) and (
                                           ...
                                           (key.route.is_ru(hardware) and fs in key.drc.fs_ru)))
            static_power_function += model.sum(model.min(1, psi_1) * hardware.static_power)
    model.add_constraint(static_power_function + dynamic_power_function <= power_cap, ctname='Power CAP')
