...
def _process_routes(graph: Graph):
    route_list = []
    identifier = 1
    ...
    for path in graph.path_list:
        ...
        for position in range(0, len(path) - 1):
            ...
            if position == count:
                ...
                new_route = Route(identifier, 'CN', path[-1], sequence, p1, p2, p3, delay_p1, delay_p2, delay_p3)
...
def find_paths(link_file_path: str, node_file_path: str, route_file_path: str, is_ring_topology: bool):
    ...
    for node in node_list:
        if node.ru == 1:
            destination_list.append(node.number)

    for destination in destination_list:
        graph.find_all_paths(0, destination)

    route_list = _process_routes(graph)
